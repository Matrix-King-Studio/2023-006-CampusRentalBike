<template>
  <div class="">
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>划定电子围栏</el-breadcrumb-item>
    </el-breadcrumb>

    <div id="mapFencecontainer" class="mapBox" style="width: 85%; height: 84%; position: absolute">
      <div class="mapBtn">
...
// 默认展示假数据回显参数
let arr = [
  { lat: 34.80506482194067, lng: 113.59760284423828 },
...
export default {
  ...
  methods: {
    ...
    // 子组件给父组件传值(确认按钮)
    sendLngLat() {
      const data = this.sendLngLatData;
      console.log(data, "传过来了");
      let data2 = [];
      data.map((value, index, arry) => {
        data2.push({ latitude: value.lat, longitude: value.lng });
      });
      console.log(data2);
      if (data == undefined) {
        data = path4;
      ...
      const res = this.$axios({
        ...
        dataType: "JSON",
        method: "post",
        url: "https://www.paper.matrix-studio.top/api/fence/create",
        data: location,
      }).then((res) => {
        console.log(res, "jiggjni");
        arr = data;
    ...
    init(props) {
      ...
      map = new qq.maps.Map(document.getElementById("mapFencecontainer"), {
        center: new qq.maps.LatLng(39.087741701843136,117.70415085713466), // 暂时默认郑州
        zoom: 15,
      });
      // 标注的生成与回显
      ...
      this.detail.forEach((item) => {
        path4.push(new qq.maps.LatLng(item.latitude, item.longitude));
      });

      console.log(path4, "hhhi");
    ...
    addpolygon() {
      ...
      qq.maps.event.addListener(
        ...
        function (event) {
          ...
          This.sendLngLatData = [...lngLat];
          // console.log(this.sendLngLatData,'kkkk'); // 获得相应的经纬度值
        }
      );
    },
    ...
    async getRailList() {
      try {
        const response = await this.$axios
          .get(" https://www.paper.matrix-studio.top/api/fence/list")
          .then((response) => {
            this.detail = response.detail;
            console.log(response.detail);
            console.log(this.detail.at(-1).locations, 99999999);
            this.detail = this.detail.at(-1).locations;
